...
fit.vd00 <- lm.rrpp(LMM~log(gmeansvd00.pop), data = rdf.vd00)
...
```{r, predictions of unknown specimens, include = FALSE}

A_reorder_unknown <- A_reorder[,,c(which(antechinusdata$Population=="?"))]

predspecgmm <- predict(PCAgmmgpa, newdata = A_reorder_unknown)
...
testtable <- matrix(c(ldapredvd00$class, ldapredd98$class, ldapredb13$class, ldapredt16$class), ncol = 4, byrow = TRUE)

predtable <- as.table(testtable)
