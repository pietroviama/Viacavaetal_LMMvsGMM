...
PCvd00red$Population <- sizefree_dfvd00.x.pop[,14]

modelsizefree_dfvd00 <- lda(Population~., data = PCvd00red, prior = c(1,1,1)/3, CV=TRUE) #warning: variables are collinear when using the entire LMM dataset, which is why we choose to regularize the data with a PCA step previous to the LDA. the LMM are indeed collinear and this is a question this research addresses.
modelsizefree_dfvd00
...
modelsizefree_dfvd00.plot <- lda(Population~., data = PCvd00red, prior = c(1,1,1)/3)
ldsvd00 <- predict(modelsizefree_dfvd00.plot)
dataEllipse(ldsvd00$x[,1], ldsvd00$x[,2], groups=as.factor(PCvd00red$Population), levels=0.95, ylim=c(-4,6), xlim=c(-7,7))

dataldavd00 <- data.frame(ldsvd00$x[,1], ldsvd00$x[,2], PCvd00red$Population)

plotldavd00 <- ggplot(dataldavd00, aes(x=ldsvd00.x...1., y=-ldsvd00.x...2., colour = PCvd00red.Population)) +
  labs(x = "LD 1 (92.27%)", y = "LD 2 (7.73%)") +
...
predsizefree_dfvd00.lda <- predict(modelsizefree_dfvd00, dfintlmlda.popx.sizefree_dfvd00)
...
PCd98red <- data.frame(PCAd98$x[,1:8]) 

PCd98red$Population <- sizefree_dfd98.x.pop[,22]

modelsizefree_dfd98 <- lda(Population~., data = PCd98red, prior = c(1,1,1)/3, CV=TRUE) #warning: variables are collinear when using the entire LMM dataset, which is why we choose to regularize the data with a PCA step previous to the LDA. the LMM are indeed collinear and this is a question this research addresses.
modelsizefree_dfd98
...
modelsizefree_dfd98.plot <- lda(Population~., data = PCd98red, prior = c(1,1,1)/3)
ldsd98 <- predict(modelsizefree_dfd98.plot)
dataEllipse(ldsd98$x[,1], ldsd98$x[,2], groups=as.factor(PCd98red$Population), levels=0.95, ylim=c(-4,6), xlim=c(-7,7))

dataldad98 <- data.frame(ldsd98$x[,1], ldsd98$x[,2], PCd98red$Population)

plotldad98 <- ggplot(dataldad98, aes(x=ldsd98.x...1., y=ldsd98.x...2., colour = PCd98red.Population)) +
  labs(x = "LD 1 (75.67%)", y = "LD 2 (4.55%)") +
  scale_color_manual(values = c("#332288", "#CC6677", "#999933")) +
  geom_point(size = 4, alpha = 0.8) +
  theme_bw() +
  stat_ellipse(size = 2, level = 0.95) +
...
predsizefree_dfd98.lda <- predict(modelsizefree_dfd98, dfintlmlda.popx.sizefree_dfd98)
...
dfgmmante <- data.frame(gpa2d, Population=antechinusdatapop$Population)
...
dataPCAvd00raw <- data.frame(-PCAintlmvd00raw$x[,1], PCAintlmvd00raw$x[,2], sizefree_dfvd00.x.pop$Population)

plotPCAvd00raw <- ggplot(dataPCAvd00raw, aes(x=X.PCAintlmvd00raw.x...1., y=PCAintlmvd00raw.x...2., colour = sizefree_dfvd00.x.pop.Population)) +
